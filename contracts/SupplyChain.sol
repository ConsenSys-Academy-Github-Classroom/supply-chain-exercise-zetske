...
contract SupplyChain {
  ...
  modifier sold(uint _sku) {
    Item storage item = items[_sku];
    require(item.state == State.Sold && item.buyer != address(0));
    _;
  }
  ...
  modifier shipped(uint _sku)  {
    Item storage item = items[_sku];
    require(item.state == State.Shipped && item.buyer != address(0));
    _;
  }
  ...
  modifier received(uint _sku) {
    Item storage item = items[_sku];
    require(item.state == State.Received && item.buyer != address(0));
    _;
  }
